...
func (r *RoleAuthDaoImpl) Insert(auth model.RoleAuth, db *gorm.DB) error {
	if db == nil {
		db = global.DBConn
	}
	err := db.Model(model.RoleAuth{}).Where("auth_id = ? and role_id = ?", auth.AuthID, auth.RoleID).FirstOrCreate(&auth).Error
	return err
}
